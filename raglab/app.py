...
from profiles import (ANSWER_PROVIDERS, EMBEDDING_PROVIDERS, KEY_ENV_INFO,
                      KIRA_BASE_URL, PLACEHOLDER_PATTERNS, SUPPORTED_ANSWER,
                      SUPPORTED_EMBEDDING, build_generator, build_lab_config,
                      collection_count, collection_name, consistent_model,
                      data_dirs, default_state, first_set_env, greeting_language,
                      greeting_reply, ingest, is_greeting, locate_text,
                      looks_like_placeholder, masked, missing_key_slots,
                      pipeline_marker, read_env_file, remove_env_assignment,
                      search_rows, slot_display, stored_index_info,
...
def action_search_chunks(state: dict) -> None:
    ...
    if not needle:
        return
    description, rows = search_rows(state)

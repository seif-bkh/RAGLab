...
def chat_config(model=CHAT_MODEL, *, thinking=False, cache_path=None, chunk_size=None,
                chunk_overlap=None, top_k=None, chunking='size', map_dir=None):
    ...
    local.CHUNKING_MODE = chunking
    if map_dir:
        local.CHUNK_MAP_DIR = Path(map_dir).expanduser().resolve()
    if chunking == 'manual':
        # A different segmentation is a different corpus, so it gets its own collection: answering from
        # whichever index happens to be on disk is how a measurement stops meaning anything. And the
        # chunk size stops describing the corpus, because the maps decide that.
        local.CHROMA_COLLECTION_NAME = 'raglab_chat_manual'
        local.CHUNK_SOURCE = 'manual maps in ' + Path(local.CHUNK_MAP_DIR).name
...
def check(model=CHAT_MODEL, *, chunking='size', map_dir=None):
    ...
    local = chat_config(model, chunking=chunking, map_dir=map_dir)
    ...
    # The size is only a description of the strategy when the strategy is size: for manual maps the
    # documents decide, and printing '640/40' there would describe something that is not happening.
    detail = (f'maps in {Path(local.CHUNK_MAP_DIR).name}' if local.CHUNKING_MODE == 'manual'
              else f'{local.CHUNK_SIZE_TOKENS}/{local.CHUNK_OVERLAP_TOKENS} tokens')
    print(f"- chunking: {local.CHUNKING_MODE} ({detail}, {local.CHUNK_SOURCE}) — the corpus is "
          're-indexed when this changes, so --check reports which one the index on disk was built with')
...
def main(argv=None):
    ...
    parser.add_argument('--chunk-map-dir', default=None, dest='map_dir',
                        help='directory of chunk maps for --chunking manual '
                             '(default: raglab/benchmarks/chunk_maps)')
    parser.add_argument('--chunking', choices=['size', 'manual'], default='size',
                        help="manual = the reviewed chunk maps in raglab/benchmarks/chunk_maps (one "
                             'subject per chunk, boundaries from the document itself); size = token '
                             'windows. A different segmentation is a different corpus, so it indexes '
                             'into its own collection: --ingest --chunking manual once, then run it.')
    ...
    if args.check and not (args.ingest or args.reset):
        return check(args.model, chunking=args.chunking, map_dir=args.map_dir)
    ...
    local = chat_config(args.model, thinking=args.thinking, chunk_size=args.chunk_tokens,
                        ...
                        chunking=args.chunking, map_dir=args.map_dir)

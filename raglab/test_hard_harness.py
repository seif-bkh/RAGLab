...
class LocalDoctor(unittest.TestCase):
    """The readiness check must not leak a key or invent a requirement the plan does not have."""
    ...
    def setUp(self):
        ...
        write_json(plan, {'phase': 'grade', 'revision': 'test-1', 'questions_per_language': 100,
                         'chunking': {'chunk_size_tokens': 640, 'chunk_overlap_tokens': 40},
                         'dataset_limit_per_language': 100,
                         'llm': {'provider': 'google', 'model': 'gemini-3.5-flash',
                                  'credential_secret': 'GEMINI_API_KEY'},
                         'candidate_llm': {'provider': 'xkiro', 'model': 'qwen/qwen3.8-max:free',
                                           'credential_secret': 'XKIRO_API_KEY'},
                         'grader_llm': {'provider': 'experiential', 'model': 'glm-5.3-flash',
                                        'credential_secret': 'EXPERIENTIAL_API_KEY'}})
        self.plan = plan

    def test_a_missing_judge_key_blocks_and_the_role_is_named_in_the_report(self):
        import hard_harness.preflight as preflight
        report = preflight.local_report(plan_path=self.plan, root=self.root, probe=False)
        self.assertEqual(report['blocking'], ['EXPERIENTIAL_API_KEY'])
        self.assertEqual(report['profiles']['grader_llm']['provider'], 'experiential')
        self.assertEqual(report['profiles']['grader_llm']['label'], 'judge')
        self.assertEqual(report['chunking']['chunk_size_tokens'], 640)
        text = preflight.format_report(report)
        self.assertIn('EXPERIENTIAL_API_KEY', text)
        self.assertIn('collect --sha', text)

    def test_the_report_never_prints_more_than_a_key_prefix(self):
        from unittest.mock import patch
        import hard_harness.preflight as preflight
        secret = 'xpl_SUPERSECRET0123456789'
        with patch.dict(os.environ, {'EXPERIENTIAL_API_KEY': secret}, clear=False):
            report = preflight.local_report(plan_path=self.plan, root=self.root, probe=False)
        text = preflight.format_report(report)
        self.assertNotIn('SUPERSECRET', text)
        self.assertIn('xpl_SUPE', text)                 # the documented eight-character mask
    ...
    def test_grading_does_not_demand_the_reference_provider_it_no_longer_uses(self):
        ...
        with patch_env({'EXPERIENTIAL_API_KEY': 'xpl_a'}):
            report = local_report(plan_path=self.plan, root=self.root, probe=False)
        self.assertNotIn('GEMINI_API_KEY', report['blocking'])

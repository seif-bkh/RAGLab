...
def run_steps() -> None:
    ...
    # NVIDIA experiments now have their own strict, exact-model workflow.
    # Never auto-discover/substitute an older embedding or translation model.
    nvidia_summary = " || NVIDIA: run nvidia_benchmark.py separately"

    CURRENT_STEP = "answer-empty-context"
    progress("\n[ci] STEP 12 — safe answer refusal with empty context (no API call)")
    import answer
    result = answer.AnswerGenerator(cfg).answer("test", [], language="en")
    check("empty context refuses without generation", result["status"] == "refused"
          and result["reason"] == "no_context" and not result["claims"])
